...
/**
 * Created by pchanumolu on 06/06/15.
 */
object SparkKafkaHBase {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("KafkaSparkStreamingHBaseWordCount")
    val ssc = new StreamingContext(sparkConf, Seconds(10))
    ssc.checkpoint("maprfs:///sparkStreamingCheckpoints")

    // configuration to connect to kafka cluster
    ...
    val wordCounts = words.map(x => (x, 1L)).reduceByKey(_ + _).reduceByKeyAndWindow(_ + _, _ - _, Minutes(5), Seconds(2), 2)
    wordCounts.print()
    ...
    wordCounts.foreachRDD(rdd => {
      ...
      rdd.saveAsTextFile("maprfs:///sparkstreaming-kafka-output")
  ...
  def connect(tableName:String) : HTableInterface = {
    val conf = getHBaseConfig()
    val hCon = new HConnectionManager.createConnection(conf)
